    ...
    def train(self, imagefolder, validfolder):
        ...
        with tf.Session(config=self.config) as sess:
            ...
            for e in range(1, self.epochs+1):
                ...
                if (tot_val_psnr / val_im_cntr) > best_psnr:
                    best_psnr = (tot_val_psnr / val_im_cntr)
                    save_path = saver.save(sess, self.ckpt_path + "edsr_ckpt")
                    print("best psnr saved.")

            print("Training finished.")

    def upscale(self, folder):
        """
        Upscales an image via model. This loads a checkpoint, not a .pb file.
        """
        for path in sorted(glob(folder+"*.png")):
            ...
            with tf.Session(config=self.config) as sess:
                ...
                cv2.imwrite("./images/"+filename, HR_image) #edsr out
                cv2.imwrite("./images/"+"bicubic_"+filename, bicubic_image) #bicubic out
                cv2.waitKey(0)
